...
这个DP来说，也是范围上的尝试。因为变化的形参，L,R都是0到n-1，所以是正方形表，因为有两个函数，所以是两张正方形表。因为不可能出现L>R的情况，**所以这两个正方形表的下三角不存在**
...
#### 写动态规划的题，首先找到暴力递归的写法去尝试，有尝试解后，如果有重复的解，那就对于可变参数，如果不讲究组织，那就用记忆化搜索的方法，如果精细化组织，那就是经典的动态规划。

拿背包问题讲，递归的时间复杂度是O（2^N),如果是记忆化搜索的方法，那么是O（N*bag），bag是这个表的大小，如果是动态规划的方法，如果**记忆化搜素查表的时候没有枚举行为**，那么动态规划的时间复杂度和记忆化搜索的时间复杂度相同，如果有的话，那么动态规划的时间复杂度比记忆化搜索更快，他会对枚举行为做优化。



==暴力递归到DP！，暴力递归的分析过程都是DP的转移方程==

==一旦写出暴力递归的解法，那么在改写DP时，只看自己的暴力递归怎么写的就行了，不用去管题目，题目和DP已经解耦了。==
...
1. 每个可变参数的类型,一定不要比int类型更复杂
2. 1)原则可以违法,让类型突破到一维线性结构,那必须是唯一可变参数
3. 如果发现1)可以违反,但不违反2),那只需要做到记忆化搜索即可
4. 可变参数的个数,能少则少
...
1）你已经有了一个不违反原则的暴力递归，而且的确存在解的重复调用2）找到哪些参数的变化会影响返回值，对每一个列出变化范围
3）参数间的所有的组合数量，意味着表大小
4）记忆化搜索的方法就是傻缓存，非常容易得到
5）规定好严格表的大小，分析位置的依赖顺序，然后从基础填写到最终解

6 ) 对于有效枚举行为的决策过程,进一步优化
...
假设有排成一行的N个位置，记为1N，N一定大于或等于2开始时机器人在其中的M位置上(M一定是1N中的一个)如果机器人来到1位置，那么下一步只能往右来到2位置;如果机器人来到N位置，那么下一步只能往左来到N-1位置;如果机器人来到中间位置，那么下一步可以往左走或者往右走;规定机器人必须走K步，最终能来到P位置(P也是1~N中的一个)的方法有多少种给定四个参数N、M、K、P，返回方法数。

ways1是进行暴力递归实现的。因为发现机器人漫步过程中会有优化，所以可以通过缓存来优化。walkCache就是一种动态规划方法，属于比较粗糙的实现，**他有个名字叫做记忆化搜索**
...
```
	...
	public static int waysCache(int N, int M, int K, int P) {
		...
		int[][] dp = new int[N+1][K+1];//第0维存放cur，第一维存放rest
...
## 假如一个数组arr=[7,3,100,50],无重复值且为正，每个值可以重复选无数次，给定一个target，问有多少种方法得到target

process1是暴力递归的版本，process2是记忆化搜素的版本，ways3是经典的DP，但是很遗憾，ways3和process2时间复杂度是一样的，都是记忆化搜素，因为有枚举行为，所以还可以再优化！
...
返回需要至少多少张贴纸可以完成这个任务。例子: str= "babac", arr = {"ba" ,"c" ,"abcd"}，至少需要两张贴纸"ba"和"abcd"，因为使用这两张贴纸，把每一个字符单独剪开，含有2个a、2个b、1个c。是可以拼出str的。所以返回2。这个题不能对记忆搜索进行经典的DP
...
public static int minStickers1(String[] stickers, String target) {
	...
	public static int process1(
		...
		for (int i = 0; i < n; i++) {
			// 枚举当前第一张贴纸是谁？
			//下面的这个if语句是说把问题转换为，对于包含第一种字符的话，至少有多少种.
...
## 咖啡杯变干净的最早时间(寻找业务限制的尝试模型)

给定一个数组，代表每个人喝完咖啡准备刷杯子的时间
只有一台咖啡机，一次只能洗一个杯子，时间耗费a，洗完才能洗下一杯每个咖啡杯也可以自己挥发干净，时间耗费b，咖啡杯可以并行挥发返回让所有咖啡杯变干净的最早完成时间三个参数: int[]arr、int a、int b



本节课只讲了“	// 方法二，洗咖啡杯的方式和原来一样，只是这个暴力版本减少了一个可变参数”这个注释下面的代码。

这个题其实也可以说是从左到右的尝试模型。不过这确实还是这个业务限制。 
