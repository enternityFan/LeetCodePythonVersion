...
sajdfkljsadklfjkl就看了就

按时间考虑到附件看两三点



![image-20230111175248662](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230111175248662.png)
...
​	红黑树著名是因为，AVL对平衡性很灵敏，SB树则对平衡性很模糊，在硬盘IO的时候，都是O(1)的操作，那么此时SB树的效果会更好些，就是模糊的更好点，模糊的意思就是虽然我每次确实都读的，但不是每次都调。。如果插入和删除行为代价非常的高，而查询的代价可以妥协，这种情况一般会用B/B+树，在硬盘上实现的，这个的读写硬盘会少。

​	redis选择跳表，而不选择AVL,SB，因为他可能有序列化的要求。
...
给一个二维数组，每行长度不一，但每行都是有序的，能不能找到一个区间，这是一个左闭右闭的区间，让每一个数组中都至少有一个数在这个区间里，求最短的区间，若有多个，那么返回开始位置最小的那个。

​	这个答案不好想，   首先做一个有序表，把二维数组每行的最小的放到有序表里面去，然后把这个有序表中的最大值和最小值组成的区间，他肯定可以满足题意，不过不一定是最短的，然后把有序表中最小的用他来自的那一行的下一个给替换掉。这个时候再把这个有序表里面的[最小值，最大值]组成这个区间，如果这个区间的间隔比之前的大，或者同样窄，那就不更新。 。
...
![image-20230102094108829](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102094108829-16726236703431.png)

​	这个老左讲的是最优解的讲法，比力扣的要简单的多。还是这个思路，如果[0...i]的累加和是100，有[0...j]是80，那么[j+1....i]就一定是20，如果必须求i结尾，有多少个子数组最后能落到【10,30】这个范围伤，其实也相当于问[0...i-1]这个范围内有多少个子数组是落在[70,90]这个范围上的。（因为0-i是100嘛假设的。）那么更一般的，**假如[0....i]的是sum，他的累加和，然后想知道有多少是落在[a,b]的，那么其实也就是想知道在[0..0],[0..1],[0...i-1]有多少个是落在[sum-a,sum-b]这个范围的。**

​	那么可以做一个结构，然后进行遍历数组，在0的时候，把0到0的累加和加到结构里去，到1的时候把0到1的累加和加到结构里去，那么这个结构里会存放之前出现的所有的累加和，到i的时候也有[0...i]的累加和sum，结构可以支持[sum-a,sum-b]范围上有哪些前缀和的数量，i结尾的子数组有多少个，答案就求出来了。这个结构只用实现两个接口，一个是add(num)，一个是query(L,R),这个其实只要实现小于num2就行的，比如如果小于R+1有a个，小于L+1有b个，那其实LR的数就是a-b。假设结构中都是不同的，那就做两个接口，一个是add(num),另外一个是less(num),如果有相同的，那么节点可以加一个字段，all去表示这这个节点为头的树上，既算相同又算不同，有多少个节点。

​	下面的图是一个例子，比如说要找小于6的有几个，我首先知道5为头，也就是一共有100个节点，然后呢，往右找，到8发现8有50个，那么我可以更新一下ans，更新为100-50，是没有问题的，然后往7的地方滑，ans不获得任何数，继续往7下面滑，如果找到6了，他有左树，那么加上他左树的值，不然不操作。**注意，因为增加了一个all的阶段，在左旋右旋这些操作的时候，不要忘记该。**。

![image-20230102100508441](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102100508441-16726251095582.png)



下面的代码里面，lessKeysize是关键接口，改有序表就是为了他，然后主逻辑是countRangeSum2.然后countWhileMergeSort这个是力扣的最佳答案，代码!比较复杂，其实看不看都行。
...
![image-20230102102054890](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102102054890-16726260567373.png)



这要是面试的话，不会让手写SB树，不过要说明怎么改。上面说的中位数是严格中位数，意思就是如果是偶数的话，那就返回(8+9)/2的值，举个例子。

​	那么这个树要允许增加重复数字，允许增加，允许删除数字，删掉的这个数字还只删一次，再增加得到第几小的k这个接口，这个问题就解了。

如下图，比如说要找第55小的数，那么在头结点，首先不会往左侧滑，因为左侧只有30个数，然后他会来到8的位置，然后这样继续滑。

![image-20230102102812021](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102102812021-16726264929124.png)



Comparable这个函数是老左自己写的，在排序的时候即考虑值也考虑index，这样的时候就可以省略all了，因为index，相当于两个key做主键。 
...
​	假如说有一个文章，我给你很多候选串，如果命中了，返回一个list，里面是候选串中哪些串命中了。

​	AC自动机本质上就是一个前缀树，但是他有别的内容。首先把候选串建立一个前缀树来，  然后每个节点都有一个fail指针，头结点的fail指针指向null,一级孩子的指向头，当来到x指针后，去设置他的通过走b到来的子节点，假如说X指针的fail指向甲，1）如果甲也有走b的路，那么这个子节点就指向甲走b到的那个指针就行，2）如果x指向null的话，就把他的子节点的fail指针设到头上去，3） 如果x的fail指针指向甲，甲没有走b的路，那么就继续调甲的fail指针，直到有b的路，那么指向这个路，否则就是遇到null，指向头结点。其实可以看出来，遇到null的前一个肯定是头，因为只有头会指向null

​	如下图所示，每一层的fail指针，到这个节点的路径，代表一个前缀字符串，fail指针的含义是必须以这个节点结尾，哪一个另外的前缀串，和我以这个节点结尾的后缀串彻底相等，并长度最大，我就直到哪去。比如说左下角的e，他是abcde，是吧，那么bcde,cde，de，e就满足他们的前缀和abcde的后缀满足相等的关系，那么最长的就是bcde是吧，然后这个e就连接到了bcde的e上了。



![image-20230102172618819](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102172618819-16726515803235.png)



​	上面两段就是分别解释流程和含义，这个流程维护含义是重要的，对于只有一次跳转，其实就是动态规划，就是怎么找到当前节点的最长，那其实就是上一个节点的最长，也就是上面第二段说的1)的情况。对于多次跳转，就是3）的情况， 其实意思就是先考虑最大的可能性，然后考虑次大的可能性，一直考虑下去，如果中一个，那就是此时的最长。下面的图就是第二种情况，对于第一次跳转是不行的，因为abcdf和bcde没有那个关系，所以一直跳，直到df才满足，所以指上、

![image-20230102174207394](F:\Study\Code\LeetCodePthonVersion\算法网课学习笔记2进阶.assets\image-20230102174207394-16726525285637.png)





利用AC自动机，把文章中不可能的开头的可能性宣告失败。
